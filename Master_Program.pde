...
// Today's aims are to add a function for calibrating the input now I'm using a real pressure transducer
// & allow 3rd window to be off scale at the bottom so that in 2-window mode the middle window can be
// right at the bottom if required
...
void draw() 
{
  ...
  } else {  // if not in calibration mode, draw the screen as usual
    drawPlot(); 
...
void serialEvent(Serial myport) { // the code to read the actual data
  ...
  if (dataStr != null ) { // if there is some data and so isn't null...
    ...
    yPos = int(map(pressure, lScale, uScale, 0, plotHeight-1)); // scales the input to the plot
